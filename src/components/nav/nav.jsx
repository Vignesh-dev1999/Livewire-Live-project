...
function DropRight({ menuList }) {
    ...
    return (
        <div className="absolute bg-slate-100 top-44  w-[76%] left-[22%] z-20 flex   pl-2 gap-1 cursor-pointer rounded-xl p-4  border border-gray-500">
            <div>
            <h5 className="text-gray-600 font-serif ">Course Categories:</h5>
            ...
            {list.map((data, index) => (
                ...
                <div><Link to={data.path} className="flex">
                    
                    {
                        ...
                        data.isActive ? <li className="flex gap-4 mt-3 ">{data["subTopics"].map((sub, index1) => (

                           <Link to={sub["path"]}>
                           
                           <p className=" bg-white p-2 border border-gray-500 mt-2 rounded-xl w-[27vh] text-sm  h-14 font-bold  ml-2  shadow-md " >{sub["name"]}</p>
                           </Link>
                        ))}</li> : <></>
                    }
                </Link>
